...
def load_metadata(path: Path) -> SheetMeta:
    ...
    if meta.columns <= 0 or meta.rows <= 0:
        raise ValueError("columns and rows must be positive")
    if meta.cell_width <= 0 or meta.cell_height <= 0:
        raise ValueError("cell dimensions must be positive")
    expected_poses = meta.columns * meta.rows
    if len(meta.poses) != expected_poses:
        raise ValueError(f"metadata has {len(meta.poses)} poses, expected {expected_poses}")
    if len(set(meta.poses)) != len(meta.poses):
...
def convert_sheet(
    ...
    *,
    ...
    image = read_rgba_png(sheet_path)
    _validate_sheet_shape(sheet_path, image, meta)
    ...
    sprites = tuple(
        ...
        for index in range(len(meta.poses))
    )
...
def render_rust_module(
    ...
    *,
    ...
    lines: list[str] = [
        ...
        f"// Source sheet: {_display_path(sheet_path)}",
...
def _validate_sheet_shape(sheet_path: Path, image: RgbaImage, meta: SheetMeta) -> None:
    expected_width = meta.columns * meta.cell_width
    expected_height = meta.rows * meta.cell_height
...
def _encode_sprite(
    ...
    *,
    ...
    pose = meta.poses[pose_index]
    cell_x = (pose_index % meta.columns) * meta.cell_width
    cell_y = (pose_index // meta.columns) * meta.cell_height
